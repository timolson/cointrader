...
/**
 * This is the base class for GeneralOrder and SpecificOrder. To create orders, see OrderBuilder or
 * BaseStrategy.order
 *
 * @author Mike Olson
 * @author Tim Olson
...
@NamedEntityGraphs({
  ...
  // @NamedEntityGraph(name = "orderWithParentFill", attributeNodes = { @NamedAttributeNode(value =
  // "parentFill", subgraph = "orderWithParentFillDetails") }, subgraphs = { @NamedSubgraph(name =
  // "orderWithParentFillDetails", attributeNodes = { @NamedAttributeNode("children") }) }),
  @NamedEntityGraph(
      name = "orderWithFills",
      attributeNodes = {@NamedAttributeNode(value = "fills")}),
  @NamedEntityGraph(
      name = "orderWithTransactions",
      attributeNodes = {@NamedAttributeNode(value = "transactions")}),
  @NamedEntityGraph(
      name = "orderWithChildOrders",
      attributeNodes = {@NamedAttributeNode(value = "orderChildren")}),
  @NamedEntityGraph(
      name = "orderWithOrderUpdates",
      attributeNodes = {@NamedAttributeNode(value = "orderUpdates")})
...
public abstract class Order extends Event {
  @Inject protected transient OrderDao orderDao;
  @Inject protected transient FillJpaDao fillDao;
  protected Integer stopAdjustmentCount = 0;
  protected static Logger log = LoggerFactory.getLogger("org.cryptocoinpartners.order");

  protected static final DateTimeFormatter FORMAT =
  ...
  public Amount getForcastedCommission() {
    if (commission == null) setForcastedCommission(FeesUtil.getCommission(this));
    return commission;
  }

  @Transient
  public Amount getForcastedMargin() {
    if (margin == null) setForcastedMargin(FeesUtil.getMargin(this));
    return margin;
  }
  ...
  @Transient
  public abstract Amount getDifferentialPrice();

  @Transient
  public abstract Amount getPeggedPrice();
  ...
  public abstract Order withDifferentialPrice(String differentialPrice);

  public abstract Order withPeggedPrice(String peggedPrice);

  public abstract Order withMarketPrice(String price);

  public abstract Order withLimitPrice(BigDecimal price);

  public abstract Order withVolume(BigDecimal volume);

  public abstract Order withDifferentialPrice(BigDecimal differentialPrice);

  public abstract Order withPeggedPrice(BigDecimal peggedPrice);

  public abstract Order withMarketPrice(BigDecimal price);

  public abstract Order withLimitPrice(DiscreteAmount price);

  public abstract Order withVolume(DiscreteAmount volume);

  public abstract Order withDifferentialPrice(DiscreteAmount differentialPrice);

  public abstract Order withPeggedPrice(DiscreteAmount peggedPrice);

  public abstract Order withMarketPrice(DiscreteAmount price);

  @ManyToOne(optional = true)
  @JoinColumn(name = "tradeable")
  public abstract Tradeable getMarket();
  ...
  public Integer getStopAdjustmentCount() {
    if (stopAdjustmentCount == null || stopAdjustmentCount == 0) stopAdjustmentCount = 1;
  ...
  public abstract void setMarket(Tradeable market);

  public synchronized void setPositionEffect(PositionEffect positionEffect) {
    this.positionEffect = positionEffect;
  }

  public synchronized void setNoLegs(Integer noLegs) {
    this.noLegs = noLegs;
  }

  public synchronized void setLegNumber(Integer legNumber) {
  ...
  public synchronized void setTargetStrategy(TargetStrategy targetStrategy) {
  ...
  public synchronized void setParentOrder(Order order) {
    if (order != null)
      log.trace(
          "Order:setParentOrder - setting parent order to {}/{} for order {} / {} . Calling class {}",
          order.getUuid(),
  ...
  public synchronized void setParentFill(Fill fill) {
    if (fill != null) {
      log.trace(
          "Order:setParentFill setting parent fill to "
              + fill.getUuid()
              + " / "
              + System.identityHashCode(fill)
              + " for order "
              + this.getUuid()
              + " / "
              + System.identityHashCode(this)
  ...
  // mappedBy(name = "`order`")
  // @OrderBy
  // @JoinColumn(name = "linkedOrder", nullable = true)
  ...
  public void persitParents() {
    ...
    // if (getPortfolio() != null) getDao().persist(getPortfolio());

    if (getParentOrder() != null) getDao().persist(getParentOrder());

    if (getLinkedOrder() != null) getDao().persist(getLinkedOrder());

    if (getParentFill() != null) getDao().persist(getParentFill());

    // for (Transaction transaction : getTransactions()) getDao().persist(transaction);

    // for (Order childOrder : getOrderChildren()) getDao().persist(childOrder);
    // for (OrderUpdate orderUpdate : getOrderUpdates()) getDao().persist(orderUpdate);
  }
  ...
  void getAllFillsByParentOrder(Order parentOrder, List allChildren) {
    synchronized (parentOrder.getOrderChildren()) {
      for (Order child : parentOrder.getOrderChildren()) {
        if (child.getFills() != null) allChildren.addAll(child.getFills());
  ...
  public static Amount getOpenAvgPrice(Collection<SpecificOrder> orders) {
    ...
    for (Order order : orders) {
      ...
      avgPrice =
          (cumVolume.plus(order.getUnfilledVolume())).isZero()
              ? avgPrice
              : ((avgPrice.times(cumVolume, Remainder.ROUND_EVEN))
                      .plus(
                          order
                              .getUnfilledVolume()
                              .times(order.getLimitPrice(), Remainder.ROUND_EVEN)))
  ...
  @Transient
  public static synchronized Amount getWorkingVolume(
      Collection<? extends Order> orders, Market market) {
  ...
  @Transient
  public static synchronized Amount getFilledVolume(
      Collection<? extends Order> orders, Market market) {
    //  if (longAvgStopPrice == null) {
    //    Amount longCumVolume = DecimalAmount.ZERO;
    //  Amount longAvgStopPrice = DecimalAmount.ZERO;
    ...
    for (Order order : orders) {
      if (order instanceof SpecificOrder) {
        synchronized (order) {
          cumVolume = cumVolume.plus(order.getVolume().minus((order.getUnfilledVolume())));
        }
        // market=order.getMarket();
      }
    }
  ...
  public static Amount getOrderOpenVolume(Collection<Order> orders, Market market) {
    ...
    //  Amount longAvgStopPrice = DecimalAmount.ZERO;
    Amount cumVolume;
    if (market == null || market.getVolumeBasis() == 0) cumVolume = DecimalAmount.ZERO;
    else cumVolume = new DiscreteAmount(0, market.getVolumeBasis());
  ...
  public static Amount getOpenAvgStopPrice(Collection<SpecificOrder> orders) {
    ...
    for (Order order : orders) {
      ...
      if (order.getStopPrice() == null
          ...
          && order.getParentOrder().getStopAmount() != null)
      ...
      else stopPrice = order.getStopPrice();
      if (stopPrice == null) continue;
      if (stopPrice != null)
        avgStopPrice =
            (cumVolume.plus(order.getUnfilledVolume())).isZero()
                ? avgStopPrice
                : ((avgStopPrice.times(cumVolume, Remainder.ROUND_EVEN))
                        .plus(order.getUnfilledVolume().times(stopPrice, Remainder.ROUND_EVEN)))
                    .divide(cumVolume.plus(order.getUnfilledVolume()), Remainder.ROUND_EVEN);
  ...
  public void loadAllChildOrdersByParentOrder(
      Order parentOrder, Map<Order, Order> orders, Map<Fill, Fill> fills) {
    log.debug(
        this.getClass().getSimpleName()
            + " - loadAllChildOrdersByParentOrder for "
            + parentOrder.getUuid()
            + "/"
    ...
    // getorder, then get all childe orders, then for each child, load child orders, so on and so
    // forth.
    ...
    //    parentOrder.getParentFill().loadAllChildOrdersByFill(parentOrder.getParentFill(), orders,
    // fills);
    ...
    try {
      ...
      orderWithChildren =
          EM.namedQueryZeroOne(
              Order.class, "Order.findOrder", withChildrenHints, parentOrder.getId());
      orderWithTransactions =
          EM.namedQueryZeroOne(Order.class, "Order.findOrder", withTransHints, parentOrder.getId());
      orderWithUpdates =
          EM.namedQueryZeroOne(
              Order.class, "Order.findOrder", withUpdatesHints, parentOrder.getId());
    } catch (Error | Exception ex) {
      log.error(
          "Order:loadAllChildOrdersByParentOrdere unable to get order for orderID: "
              + parentOrder.getUuid()
              + ". Full stack trace ",
          ex);
      return;
    ...
    if (orderWithFills != null
        && orderWithFills.getFills() != null
        && Hibernate.isInitialized(orderWithFills.getFills())
      ...
      for (Fill fill : orderWithFills.getFills()) {
        ...
        if (fill.getOrder().getParentOrder() != null
            && fill.getOrder().getParentOrder().getPortfolio().equals(parentOrder.getPortfolio()))
          fill.getOrder().getParentOrder().setPortfolio(parentOrder.getPortfolio());
        if (fill.getPosition() != null
            && parentOrder.getPortfolio().getPositions().contains(fill.getPosition())) {
        ...
        if (!fills.containsKey(fill)) {
          fills.put(fill, fill);
          log.debug(
              "Order loadAllChildOrdersByParentOrder loading all child order for fill"
                  + fill.getUuid()
                  + " for order "
                  + orderWithFills.getUuid()
                  + ". Calling class "
                  + Thread.currentThread().getStackTrace()[2]);
      ...
      log.debug(
          "Order:loadAllChildOrdersByParentOrder - Setting fills for order"
              + parentOrder.getUuid()
              + "/"
              + System.identityHashCode(parentOrder)
              + " to fills from "
              + orderWithFills.getUuid()
              + "/"
              + System.identityHashCode(orderWithFills));
      parentOrder.setFills(orderWithFills.getFills());
    } else parentOrder.setFills(new CopyOnWriteArrayList<Fill>());

    if (orderWithUpdates != null
        && orderWithUpdates.getOrderUpdates() != null
        && Hibernate.isInitialized(orderWithUpdates.getOrderUpdates())
        ...
        if (!orders.containsKey(orderUpdate.getOrder())) {
          ...
          if (orderUpdate.getOrder().getParentOrder() != null
              && orderUpdate
                  ...
                  .getParentOrder()
                  .getPortfolio()
                  .equals(parentOrder.getPortfolio()))
            orderUpdate.getOrder().getParentOrder().setPortfolio(parentOrder.getPortfolio());

          orderUpdate
              .getOrder()
              .loadAllChildOrdersByParentOrder(orderUpdate.getOrder(), orders, fills);
        } else orderUpdate.setOrder(orders.get(orderUpdate.getOrder()));
      }

      parentOrder.setOrderUpdates(orderWithUpdates.getOrderUpdates());
    } else parentOrder.setOrderUpdates(new CopyOnWriteArrayList<OrderUpdate>());

    if (orderWithTransactions != null
        && orderWithTransactions.getTransactions() != null
        && Hibernate.isInitialized(orderWithTransactions.getTransactions())
      ...
      for (Transaction transaction : orderWithTransactions.getTransactions()) {
        ...
        if (!orders.containsKey(transaction.getOrder())) {
          ...
          transaction
              .getOrder()
              .loadAllChildOrdersByParentOrder(transaction.getOrder(), orders, fills);
        } else transaction.setOrder(orders.get(transaction.getOrder()));
      }

      parentOrder.setTransactions(orderWithTransactions.getTransactions());
    } else parentOrder.setTransactions(new CopyOnWriteArrayList<Transaction>());

    if (orderWithChildren != null
        && orderWithChildren.getOrderChildren() != null
        && Hibernate.isInitialized(orderWithChildren.getOrderChildren())
      ...
      for (Order order : orderWithChildren.getOrderChildren()) {
        ...
        if (order.getPortfolio().equals(parentOrder.getPortfolio())) {
          log.debug(
              "Order:loadAllChildOrdersByParentOrder - Setting order "
                  + order.getUuid()
                  + "/"
                  + System.identityHashCode(order)
                  + " portfolio to: "
        ...
        if (order.getParentOrder() != null
            && order.getParentOrder().getPortfolio().equals(parentOrder.getPortfolio())) {
          log.debug(
              "Order:loadAllChildOrdersByParentOrder - Setting parent order "
                  + order.getParentOrder().getUuid()
                  + "/"
                  + System.identityHashCode(order.getParentOrder())
                  + " portfolio to: "
                  + parentOrder.getPortfolio()
                  + "/"
                  + System.identityHashCode(parentOrder.getPortfolio()));
        ...
        if (order.getParentFill() != null
            && order.getParentFill().getPortfolio().equals(parentOrder.getPortfolio())) {
          log.debug(
              "Order:loadAllChildOrdersByParentOrder - Setting parent fill "
                  + order.getParentFill().getUuid()
                  + "/"
                  + System.identityHashCode(order.getParentFill())
                  + " portfolio to: "
                  + parentOrder.getPortfolio()
                  + "/"
                  + System.identityHashCode(parentOrder.getPortfolio()));
        ...
        if (!orders.containsKey(order)) {
          ...
          log.debug(
              "Order:loadAllChildOrdersByParentOrder - Loading child orders for: "
                  + order.getUuid());
          order.loadAllChildOrdersByParentOrder(order, orders, fills);
        } else orderWithChildren.getOrderChildren().set(index, orders.get(order));
        index++;
      }
      log.debug(
          "Order:loadAllChildOrdersByParentOrder - setting orderChildren to "
              + System.identityHashCode(orderWithChildren.getOrderChildren())
              + "for order "
              + parentOrder.getUuid()
              + " /"
              + System.identityHashCode(parentOrder));
      ...
      log.debug(
          "Order:loadAllChildOrdersByParentOrder - setting orderChildren to new array list for order "
              + parentOrder.getUuid()
              + " /"
              + System.identityHashCode(parentOrder));
    ...
    if (orders.containsKey(parentOrder.getParentOrder())) {
      log.debug(
          "Order:loadAllChildOrdersByParentOrder -setting parent order for"
              + parentOrder.getUuid()
              + " / "
              + System.identityHashCode(parentOrder)
              + " to parent order "
              + orders.get(parentOrder.getParentOrder()).getUuid()
              + " / "
    ...
    if (fills.containsKey(parentOrder.getParentFill())) {
      log.debug(
          "Order:loadAllChildOrdersByParentOrder - setting parent fill for "
              + parentOrder.getUuid()
              + " / "
              + System.identityHashCode(parentOrder)
              + " to fill  "
              + fills.get(parentOrder.getParentFill()).getUuid()
              + " / "
  ...
  void getAllGeneralOrderByParentOrder(Order paretnOrder, Set<Order> allChildren) {
    ...
    synchronized (paretnOrder.getOrderChildren()) {
      for (Order child : paretnOrder.getOrderChildren()) {
        if (child instanceof GeneralOrder) if (!allChildren.contains(child)) allChildren.add(child);
        getAllGeneralOrderByParentOrder(child, allChildren);
      }
    }
  }

  public void getAllParentOrdersByOrder(Order order, Collection<Order> allParents) {

    ArrayList<EntityBase> parents = new ArrayList<EntityBase>();
    getParents(order.getParent(), parents);

    for (EntityBase parent : parents) if (parent instanceof Order) allParents.add((Order) parent);
  }

  public void getAllSpecificOrderByParentOrder(Order parentOrder, Set<Order> allChildren) {

    synchronized (parentOrder.getOrderChildren()) {
      for (Order child : parentOrder.getOrderChildren()) {
        if (child instanceof SpecificOrder)
          if (!allChildren.contains(child)) allChildren.add(child);
  ...
  public Order withStopAmount(BigDecimal price) {
    ...
    if (getFillType().isTrigger()) {
      if (price.compareTo(BigDecimal.ZERO) == 0) return this;
  ...
  public Order withLastBestPrice(BigDecimal bestLastPrice) {
    ...
    if (bestLastPrice.compareTo(BigDecimal.ZERO) == 0
        || bestLastPrice.longValue() == Long.MAX_VALUE
        || bestLastPrice.longValue() == Long.MIN_VALUE) return this;
  ...
  public Order withStopPercentage(double stopPercentage) {
    ...
    if (getFillType().isTrigger()) {
      if (stopPercentage == 0) return this;
      this.setStopPercentage(stopPercentage);
      this.setStopAmount(
          DecimalAmount.of(getLimitPrice().times(stopPercentage, Remainder.ROUND_EVEN)));
      return this;
    }
  ...
  public Order withMinimumPricePercentage(Double minimumPricePercentage) {

    this.setMinimumPricePercentage(minimumPricePercentage);
    return this;
  }

  public Order withPercentageOfVolume(Double percentageOfVolume) {

    this.setPercentageOfVolume(percentageOfVolume);
    return this;
  }

  public Order withPercentageOfVolumeInterval(Double withPercentageOfVolumeInterval) {

    this.setPercentageOfVolumeInterval(withPercentageOfVolumeInterval);
    return this;
  }

  public Order withVariance(Double variance) {

    this.setVariance(variance);
    return this;
  }

  public Order withPayUpTicks(Integer payUpTicks) {

    this.setPayUpTicks(payUpTicks);
    return this;
  }

  public Order withRatioQuantity(Double ratioQuantity) {

    this.setRatioQuantity(ratioQuantity);
    return this;
  }
  ...
  public Order withTargetPercentage(double targetPercentage) {
    ...
    if (getFillType().isTrigger()) {
      if (targetPercentage == 0) return this;
      this.setTargetPercentage(targetPercentage);
      this.setTargetAmount(
          DecimalAmount.of(getLimitPrice().times(targetPercentage, Remainder.ROUND_EVEN)));
      return this;
    }
  ...
  public Order withTriggerInterval(double triggerInterval) {
    ...
    if (getFillType().isTrigger()) {
      if (triggerInterval == 0) return this;
  ...
  public Order withTriggerTime(Instant triggerTime) {
  ...
  public Order withLinkedOrder(Order linkedOrder) {
    this.setLinkedOrder(linkedOrder);
    return this;
  }
  ...
  public Order withNoLegs(Integer noLegs) {
    this.setNoLegs(noLegs);
    return this;
  }

  public Order withLegNumber(Integer legNumber) {
  ...
  public Order withTargetStrategy(TargetStrategy targetStrategy) {
  ...
  public synchronized void prePersist() {
    if (getDao() != null) {
      ...
      if (getLinkedOrder() != null) getDao().merge(getLinkedOrder());
  ...
  public synchronized void persit() {
    ...
    //  List<Order> duplicate = orderDao.queryList(Order.class, "select o from Order o where o=?1",
    // this);
    ...
    try {
      ...
      log.debug(
          "Order - Persist : Persit of Order {} called from class {} ",
          this.getUuid(),
          Thread.currentThread().getStackTrace()[2]);
      ...
      log.error(
          "Unable to perform request in {}:persist, full stack trace follows:{}",
          this.getClass().getSimpleName(),
          ex);
    }
    // else
  ...
  public Transaction getReservation() {
    synchronized (getTransactions()) {
      for (Transaction tran : getTransactions()) {
        ...
        if (tran.getType() == (TransactionType.BUY_RESERVATION)
            || tran.getType() == (TransactionType.SELL_RESERVATION)) {
          return tran;
        }
      }
    }
  ...
  public synchronized void addChildOrder(Order order) {
    log.debug(
        this.getClass().getSimpleName()
            + "addChildOrder - adding child order "
            + order.getUuid()
            + "/"
            + System.identityHashCode(order)
            + " to order "
  ...
  public DecimalAmount getAverageFillPrice() {
    if (!hasFills()) return DecimalAmount.ZERO;
    ...
    return (sumProduct.equals(BigDecimal.ZERO))
        ? DecimalAmount.ZERO
        : new DecimalAmount(sumProduct.divide(volume, Amount.mc));
  }
  ...
  public synchronized void setLinkedOrder(Order linkedOrder) {
    // setLinkedOrderId(linkedOrder.getId());
    this.linkedOrder = linkedOrder;
  }

  /*  public synchronized void setLinkedOrderId(Long linkedOrderId) {
  ...
  public synchronized void setTriggerTime(Instant time) {
    this.triggerTime = time;
  }

  public Integer getPlacementCount() {
    if (placementCount == 0) return 1;
    return placementCount;
  }

  public synchronized void setPlacementCount(Integer placementCount) {
  ...
  public Instant getExpiryTime() {
    if (getTimeToLive() != 0) return getTime().plus(getTimeToLive());
    else return null;
  }
  ...
  public Double getMinimumPricePercentage() {
    return minimumPricePercentage;
  }

  public Double getPercentageOfVolume() {
    return percentageOfVolume;
  }

  @Nullable
  public Double getPercentageOfVolumeInterval() {
    return percentageOfVolumeInterval;
  }
  ...
  public synchronized void setMinimumPricePercentage(Double minimumPricePercentage) {
    this.minimumPricePercentage = minimumPricePercentage;
  }

  public synchronized void setPercentageOfVolume(Double percentageOfVolume) {
    this.percentageOfVolume = percentageOfVolume;
  }

  public synchronized void setPercentageOfVolumeInterval(Double percentageOfVolumeInterval) {
    this.percentageOfVolumeInterval = percentageOfVolumeInterval;
  }
  ...
  public synchronized void setPayUpTicks(Integer payUpTicks) {
    this.payUpTicks = payUpTicks;
  }

  public synchronized void setRatioQuantity(Double ratioQuantity) {
  ...
  protected Integer noLegs = 0;
  protected Integer legNumber = 0;

  // @Column(name = "linked_order_id")
  protected Order linkedOrder;
  // protected Long linkedOrderId;
  ...
  protected boolean
      emulation; // ("allow order type emulation" [default, true] or "only use exchange's native
  // functionality")
  protected boolean usePosition = false;
  protected Order parentOrder;
  protected Fill parentFill;
  protected Integer placementCount = 1;
  protected Double minimumPricePercentage = 0.0;
  protected Double percentageOfVolume = 1.0;
  protected Double percentageOfVolumeInterval;
  protected Double variance = 0.0;
  protected Integer payUpTicks = 0;
  protected Double ratioQuantity = -1.0;
