...
/**
 * A Holding represents an Asset on an Exchange. It does not specify how much of the asset is held.
 * See Position
 *
 * @author Tim Olson
 */
...
public abstract class Holding extends EntityBase {
  @Inject protected transient HoldingDao holdingDao;
  @Inject protected static transient BalanceFactory balanceFactory;
  ...
  public static Holding forSymbol(String symbol) {
    log.debug(
        "Holding - forSymbol: called from class " + Thread.currentThread().getStackTrace()[2]);
    ...
    log.debug(
        "Holding - forSymbol creating balance for exchange "
            + matcher.group(1)
            + " asset: "
            + matcher.group(2));

    return balanceFactory.create(
        Exchange.forSymbol(matcher.group(1)), Asset.forSymbol(matcher.group(2)));
  }
