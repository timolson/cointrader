...
/**
 * A Fill represents some completion of an Order. The volume of the Fill might be less than the
 * requested volume of the Order
 *
 * @author Tim Olson
 */
@Entity
...
@NamedEntityGraphs({
  ...
  @NamedEntityGraph(
      name = "fillWithChildOrders",
      attributeNodes = {@NamedAttributeNode("fillChildOrders")}),
  @NamedEntityGraph(
      name = "fillWithTransactions",
...
// })
// , subgraph = "childrenWithFills") }, subgraphs = { @NamedSubgraph(name = "childrenWithFills",
// attributeNodes = { @NamedAttributeNode("fills") }) })
// @NamedEntityGraph(name = "fillWithChildOrders", attributeNodes = { @NamedAttributeNode(value =
// "fillChildOrders", subgraph = "childrenWithFills") }, subgraphs = { @NamedSubgraph(name =
// "childrenWithFills", attributeNodes = { @NamedAttributeNode("fills") }) })
// @NamedSubgraph(name = "fills", attributeNodes = @NamedAttributeNode(value = "fills", subgraph =
// "order"))
// ,@NamedSubgraph(name = "order", attributeNodes = @NamedAttributeNode("order"))
public class Fill extends RemoteEvent {
  ...
  @Inject protected transient FillDao fillDao;
  protected static Logger log = LoggerFactory.getLogger("org.cryptocoinpartners.fill");

  @Inject
  public Fill(
      @Assisted SpecificOrder order,
      @Assisted("fillTime") Instant time,
      @Assisted("fillTimeReceived") Instant timeReceived,
      @Assisted Market market,
      @Assisted("fillPriceCount") long priceCount,
      @Assisted("fillVolumeCount") long volumeCount,
  ...
  public Fill(
      ...
      @Assisted Instant timeReceived,
      @Assisted Market market,
      @Assisted long priceCount,
      @Assisted long volumeCount,
      @Assisted Amount commission,
      @Assisted String remoteKey) {
    super(time, timeReceived, remoteKey);
    this.getUuid();
    this.fillChildOrders = new CopyOnWriteArrayList<Order>();
  ...
  public synchronized void persit() {
    ...
    // List<Fill> duplicate = fillDao.queryList(Fill.class, "select f from Fill f where f=?1",
    // this);
    ...
    //    System.out.println("Unable to perform request in " + this.getClass().getSimpleName() +
    // ":persist, full stack trace follows:" + ex);
    // ex.printStackTrace();
  ...
  public boolean isLong() {
    if (getOpenVolume() == null) return getOpenVolume().isZero();
    return getOpenVolume().isPositive();
  }

  public synchronized void addChildOrder(Order order) {
    if (!getFillChildOrders().contains(order)) getFillChildOrders().add(order);
  }
  ...
  public List<Order> getFillChildOrders() {
    if (fillChildOrders == null) fillChildOrders = new CopyOnWriteArrayList<Order>();
    //  synchronized (//) {
  ...
  protected synchronized void setFillChildOrders(List<Order> children) {
    // log.trace("Fill:setFillChildOrders setting child orders [ ] to " +
    // System.identityHashCode(children) + " for fill " + getId() + " / "
    //   + System.identityHashCode(this) + "Calling class " +
  ...
  public List<Transaction> getTransactions() {
    if (transactions == null) transactions = new CopyOnWriteArrayList<Transaction>();
  ...
  public synchronized void addTransaction(Transaction transaction) {
    ...
    synchronized (getTransactions()) {
      if (!getTransactions().contains(transaction)) getTransactions().add(transaction);
    }
  ...
  public DiscreteAmount getPrice() {
    //  if (getPriceCount() == 0)
    //    return null;
    if (market.getPriceBasis() == 0) return null;
  ...
  public long getStopPriceCount() {
    if (getOpenVolumeCount() != 0) return stopPriceCount;
    else return 0;
  }

  public long getOriginalStopPriceCount() {
    if (getOpenVolumeCount() != 0) return originalStopPriceCount;
    else return 0;
  }
  ...
  public long getOpenVolumeCount() {
    if (openVolumeCount != null) return openVolumeCount.get();
    return 0L;
  }

  @Transient
  public Amount getCommission() {
    if (commission == null) setCommission(FeesUtil.getCommission(this));
    return commission;
  }
  ...
  public Amount getMargin() {
    if (margin == null) setMargin(FeesUtil.getMargin(this));
  ...
  public PositionType getPositionType() {
    ...
    if (positionType == null)
      if (getVolume().isPositive()) return PositionType.LONG;
      else return PositionType.SHORT;
    // If i have children and all the children are fully filled, then I set to flat
    if (hasChildren() && getUnfilledVolume().isZero()) return PositionType.FLAT;
    else return positionType;
  ...
  public FillType getFillType() {
    if (getOrder() != null) return getOrder().getFillType();
    return null;
  }
  ...
  public String toString() {
    ...
    return "Id="
        ...
        + SEPARATOR
        + "PositionType="
        + (getPositionType() != null ? getPositionType() : "")
        + SEPARATOR
        + "Market="
        + (market != null ? market : "")
        + SEPARATOR
        + "Price="
        + (getPrice() != null ? getPrice() : "")
        + SEPARATOR
        + "Price Count="
        + getPriceCount()
        + SEPARATOR
        + "Volume="
        + (getVolume() != null ? getVolume() : "")
        + SEPARATOR
        + "Unfilled Volume="
        + (hasChildren() ? getUnfilledVolume() : "")
        + SEPARATOR
        + "Open Volume="
        + (getOpenVolume() != null ? getOpenVolume() : "")
        + SEPARATOR
        + "Open Volume Count="
        + getOpenVolumeCount()
        + SEPARATOR
        + "Commission="
        + (getCommission() != null ? getCommission() : "")
        + SEPARATOR
        + "Position Effect="
        + (getPositionEffect() != null ? getPositionEffect() : "")
        + SEPARATOR
        + "Position="
        + (getPosition() != null ? getPosition().getId() + "/" + getPosition().getId() : "")
        + SEPARATOR
        + "Comment="
        + (getOrder() != null && getOrder().getComment() != null ? getOrder().getComment() : "")
        + SEPARATOR
        + "Order="
        + (getOrder() != null ? getOrder().getId() + "/" + getOrder().getUuid() : "")
        + SEPARATOR
        + "Order group="
        + getOrderGroup()
        + SEPARATOR
        + "Parent Fill="
        + ((getOrder() != null && getOrder().getParentFill() != null)
            ? getOrder().getParentFill().getId() + "/" + getOrder().getParentFill().getUuid()
            : "");
  }
  ...
  public synchronized void setPositionType(PositionType positionType) {
    if (this.positionType == (PositionType.FLAT))
      log.debug(
          "{}:setPositionType - Setting postion type from FLAT to {}",
          this.getClass().getSimpleName(),
          positionType);
  ...
  public Amount getUnfilledVolume() {
    ...
    if (getVolume().isZero()) return DecimalAmount.ZERO;
    for (Order childOrder : getFillChildOrders()) {
      ...
      for (Fill childFill : allChildFills) {
        ...
        if (getVolume() != null
                && !getVolume().isZero()
                && (childFill.getVolume().isPositive() && getVolume().isNegative())
            || (childFill.getVolume().isNegative() && getVolume().isPositive()))
          filled = filled.plus(childFill.getVolume());
      }
      unfilled =
          (getVolume().isNegative())
              ? (getVolume().abs().minus(filled.abs())).negate()
              : getVolume().abs().minus(filled.abs());
    }
    return unfilled;
  }
  ...
  public Amount getWorkingVolume(OrderService orderService) {
    ...
    if (getVolume().isZero()) return DecimalAmount.ZERO;
    for (Order childOrder : getFillChildOrders()) {
      if (!orderService.getOrderState(childOrder).isOpen()) continue;
      ArrayList<Fill> allChildFills = new ArrayList<Fill>();
      childOrder.getAllFillsByParentOrder(childOrder, allChildFills);

      for (Fill childFill : allChildFills) {
        if (!orderService.getOrderState(childFill.getOrder()).isOpen()) continue;
        if (getVolume() == null || childFill.getVolume() == null)
          System.out.println("null fill volume");
        if (getVolume() != null
                && !getVolume().isZero()
  ...
  public void loadAllChildOrdersByFill(
      Fill parentFill, Map<Order, Order> orders, Map<Fill, Fill> fills) {
    ...
    log.trace(
        "Fill:loadAllChildOrdersByFill loading child order for fill {} . Calling class {}",
        parentFill.getUuid(),
        Thread.currentThread().getStackTrace()[2]);
    try {

      log.trace(
          "Fill:loadAllChildOrdersByFill loading child order for parent order: {}/{}  of fill{}",
          parentFill.getOrder().getUuid(),
          System.identityHashCode(parentFill.getOrder()),
          parentFill.getUuid());

      parentFill.getOrder().loadAllChildOrdersByParentOrder(parentFill.getOrder(), orders, fills);

      log.trace(
          "Fill:loadAllChildOrdersByFill order children for parent fill: {}", parentFill.getUuid());

      fillWithChildren =
          EM.namedQueryZeroOne(
              Fill.class, "Fill.findFill", withChildOrderHints, parentFill.getId());
      fillWithTransactions =
          EM.namedQueryZeroOne(
              Fill.class, "Fill.findFill", withTransactionsHints, parentFill.getId());
      //   orderWithChildren = EM.namedQueryZeroOne(Order.class, "Fill.findFill",
      // withChildOrderHints, parentFill.getOrder());

    } catch (Error | Exception ex) {
      log.error(
          "Fill:loadAllChildOrdersByFill unable to get fill for fillID: {} {}",
          parentFill.getUuid(),
          ex);
      return;
    }
    ...
    if (parentFill.getPosition() != null
        && parentFill.getPortfolio().getPositions().contains(parentFill.getPosition())) {
    ...
    if (fillWithChildren != null
        && fillWithChildren.getFillChildOrders() != null
        && fillWithChildren.getUuid().equals(parentFill.getUuid())
        && Hibernate.isInitialized(fillWithChildren.getFillChildOrders())) {
      int index = 0;
      for (Order order : fillWithChildren.getFillChildOrders()) {
        log.debug(
            "Fill:loadAllChildOrdersByFill loading child order {}/{}  for fill {}",
            order.getUuid(),
        ...
        if (order.getParentOrder() != null
            && order.getParentOrder().getPortfolio().equals(parentFill.getPortfolio()))
          order.getParentOrder().setPortfolio(parentFill.getPortfolio());
        if (order.getParentFill() != null
            && order.getParentFill().getPortfolio().equals(parentFill.getPortfolio()))
        ...
        if (!orders.containsKey(order)) {
          ...
          log.info(
              "Fill:loadAllChildOrdersByFill Order not in order map, loading child order from database {}/{}  for fill {}",
              order.getUuid(),
              System.identityHashCode(order),
              parentFill.getUuid());
        ...
        log.debug(
            "Fill:loadAllChildOrdersByFill setting child order index {} to order {}/{}  for fill{}/{}",
            index,
            order.getUuid(),
            System.identityHashCode(order),
            parentFill.getUuid(),
            System.identityHashCode(parentFill));

        fillWithChildren.getFillChildOrders().set(index, orders.get(order));

        log.debug(
            "Fill:loadAllChildOrdersByFill setting parent fill to {}/{} for order {}/{}",
            parentFill.getUuid(),
            System.identityHashCode(parentFill),
            order.getUuid(),
            System.identityHashCode(order));
      ...
      log.debug(
          "Fill:loadAllChildOrdersByFill setting children to {} for fill {}",
          fillWithChildren.getFillChildOrders().hashCode(),
          parentFill.getUuid());

      parentFill.setFillChildOrders(fillWithChildren.getFillChildOrders());
    } else parentFill.setFillChildOrders(new CopyOnWriteArrayList<Order>());

    if (orders.containsKey(getOrder())) setOrder((SpecificOrder) orders.get(getOrder()));
      ...
      log.debug(
          "Fill:loadAllChildOrdersByFill order: {}/{}  not in order map, loading child orders  for fill {}",
          getOrder().getUuid(),
          System.identityHashCode(getOrder()),
          parentFill.getUuid());
    ...
    if (fillWithTransactions != null
        && fillWithTransactions.getTransactions() != null
        && Hibernate.isInitialized(fillWithTransactions.getTransactions())
        && fillWithTransactions.getUuid().equals(parentFill.getUuid())) {

      for (Transaction transaction : fillWithTransactions.getTransactions()) {

        log.debug(
            "Fill:loadAllChildOrdersByFill loading transactions {}/{} for fill {} ",
            transaction.getUuid(),
            System.identityHashCode(order),
            parentFill.getUuid());
        ...
        } else transaction.setFill(fills.get(transaction.getFill()));
        ...
        if (transaction.getOrder() != null && !orders.containsKey(transaction.getOrder())) {
          ...
          if (transaction.getOrder().getParentOrder() != null
              && transaction
                  ...
                  .getParentOrder()
                  .getPortfolio()
                  .equals(parentFill.getPortfolio()))
    ...
    } else parentFill.setTransactions(new CopyOnWriteArrayList<Transaction>());
  }
  ...
  public void getAllParentFillsByFill(Fill fill, Collection<Fill> allParents) {
    ...
    for (EntityBase parent : parents) if (parent instanceof Fill) allParents.add((Fill) parent);
  }
  ...
  public synchronized void setTrailingStopPriceCount(long trailingStopPriceCount) {
    if (trailingStopPriceCount != 0) this.trailingStopPriceCount = trailingStopPriceCount;
  }
  ...
  public synchronized void setOpenVolumeCount(long openVolumeCount) {
    // log.trace(this.getClass().getSimpleName() + " : setOpenVolumeCount to " + openVolumeCount +
    // "for " + this.getId() + "called from stack "
    //   + sun.reflect.Reflection.getCallerClass(2));
    ...
    /*		if (this.position != null)
    			this.position.reset();
    */ if (this.openVolumeCount == null) this.openVolumeCount = new AtomicLong(openVolumeCount);
    else this.openVolumeCount.set(openVolumeCount);
  ...
  public synchronized void delete() {
    ...
    log.debug(
        "Fill - Delete : Delete of Fill {} called from class {}",
        this.getUuid(),
        Thread.currentThread().getStackTrace()[2]);
  ...
  public void persitParents() {
    ...
    // for (Order childorder : getFillChildOrders()) getDao().persist(childorder);
