...
public class PortfolioManager extends EntityBase implements Context.AttachListener {
  ...
  protected static boolean ignoreCosts =
      (ConfigUtil.combined() != null)
          ? ConfigUtil.combined().getBoolean("ignore.costs", false)
          : false;

  // private BasicPortfolioService portfolioService;

  // todo we need to get the tradeable portfolio separately from the "reserved" portfolio (assets
  ...
  private void updateReservation(OrderUpdate update) {
    ...
    if (update.getOrder().getPortfolio().equals(portfolio)) {
      ...
      if (reservation != null && update.getState() != OrderState.NEW) {
        if (reservation.getType() == (TransactionType.BUY_RESERVATION)
            || reservation.getType() == (TransactionType.SELL_RESERVATION)) {
          Amount price =
              (update.getOrder().getLimitPrice() == null)
                  ? ((update.getOrder().getVolume().isNegative())
                      ? quotes.getLastBidForMarket(update.getOrder().getMarket()).getPrice()
                      : quotes.getLastAskForMarket(update.getOrder().getMarket()).getPrice())
                  : update.getOrder().getLimitPrice();
          Amount updateAmount =
              reservation.getType() == (TransactionType.BUY_RESERVATION)
                  ? (update.getOrder().getUnfilledVolume().times(price, Remainder.ROUND_EVEN))
                      .negate()
        ...
        default:
          log.error(
              this.getClass().getSimpleName()
                  + ":removeReservation - Called from class "
  ...
  //  @When("@Priority(8) select * from Transaction where NOT (Transaction.type=TransactionType.BUY
  // and Transaction.type=TransactionType.SELL)")
  ...
  public void updatePortfolio(Transaction transaction) {
    ...
    //	Transaction tans = new Transaction(this, position.getExchange(), position.getAsset(),
    // TransactionType.CREDIT, position.getVolume(),
    ...
    if (transaction.getPortfolio().equals(portfolio)) {
      ...
      if (transaction.getExchange() != null && (transaction.getType().isBookable())) {
        ...
        log.debug(
            this.getClass().getSimpleName()
                + "- updatePortfolio determing updates to exchange "
        ...
        if ((transaction.getType() == TransactionType.BUY
          ...
          Amount currentBalance =
              (transaction.getExchange().getBalances().isEmpty()
                      || transaction.getExchange().getBalances().get(commisionCurrency) == null)
                  ? DecimalAmount.ZERO
                  : transaction.getExchange().getBalances().get(commisionCurrency).getAmount();

          if ((transaction.getExchange().getBalances().isEmpty()
              || transaction.getExchange().getBalances().get(commisionCurrency) == null)) {
            log.debug(
                this.getClass().getSimpleName()
                    + "- updatePortfolio creating buy/sell for "
                    + transaction.getType()
                    + " balance for exchange "
                    + transaction.getExchange()
                    + " asset: "
                    + commisionCurrency
                    + " amount: "
                    + currentBalance.plus(commisionAmount));
            ...
            newBalance =
                balanceFactory.create(
                    ...
                    commisionCurrency,
                    currentBalance.plus(commisionAmount));
            transaction.getExchange().getBalances().put(commisionCurrency, newBalance);
            newBalance.persit();
            log.debug(
                this.getClass().getSimpleName()
                    + "- updatePortfolio new balance "
                    + newBalance
                    + " added to balances "
                    + transaction.getExchange().getBalances()
                    + " for "
            ...
            transaction
                ...
                .setAmount(updatedBalanceAmount);

            transaction.getExchange().getBalances().get(commisionCurrency).merge();
            log.debug(
                this.getClass().getSimpleName()
                    + "- updatePortfolio updated balance "
                    + transaction.getExchange().getBalances().get(commisionCurrency)
                    + " in balances "
                    + transaction.getExchange().getBalances()
                    + " for "
                    + transaction.getType()
                    ...
                    + updatedBalanceAmount
                    + "/"
                    + updatedBalanceAmount.asBigDecimal()
                    + " and transactino amount "
                    + commisionCurrency
                    + "/"
                    + commisionAmount.asBigDecimal());
          }
          transaction.getExchange().merge();
        } else if (!(transaction.getType() == TransactionType.BUY
            || transaction.getType() == TransactionType.SELL)) {
          if ((transaction.getExchange().getBalances().isEmpty()
              || transaction.getExchange().getBalances().get(transaction.getCurrency()) == null)) {
            Amount currentBalance =
                (transaction.getExchange().getBalances().isEmpty()
                        || transaction.getExchange().getBalances().get(transaction.getCurrency())
                            == null)
                    ? DecimalAmount.ZERO
                    : transaction
                        .getExchange()
            ...
            log.debug(
                this.getClass().getSimpleName()
                    + "- updatePortfolio creating balance for"
                    + transaction.getType()
                    + " on exchange "
                    + transaction.getExchange()
                    + " asset: "
            ...
            newBalance =
                (transaction.getType().isDebit()
                    ? balanceFactory.create(
                        transaction.getExchange(),
                        transaction.getCurrency(),
                        (currentBalance.minus(transaction.getAmount())))
                    : balanceFactory.create(
                        transaction.getExchange(),
                        transaction.getCurrency(),
            ...
            log.debug(
                this.getClass().getSimpleName()
                    ...
                    + transaction.getAmount()
                    + "/"
                    + transaction.getAmount().asBigDecimal());
          }
          transaction.getExchange().merge();
        }
      }
      for (Asset asset : transaction.getExchange().getBalances().keySet())
        if (!asset
            .getSymbol()
            .equals(transaction.getExchange().getBalances().get(asset).getAsset().getSymbol()))
          log.debug("balance inccorectly placed");

      log.trace("transaction: {}  Proccessed.", transaction);
  ...
  public static class DataSubscriber {
    ...
    public void update(Long Timestamp, Portfolio portfolio) {
      ...
      PortfolioService portfolioService =
          portfolio.context.getInjector().getInstance(PortfolioService.class);
      ...
      log.info(
          "Date: "
              ...
              + portfolioService
                  ...
                  .plus(portfolioService.getBaseUnrealisedPnL(portfolio.getBaseAsset()))
                  .minus(portfolioService.getBaseComissionAndFee(portfolio.getBaseAsset()))
              + " (Cash Balance:"
              + portfolioService.getBaseCashBalance(portfolio.getBaseAsset())
              + " Realised PnL (M2M):"
              + portfolioService.getBaseRealisedPnL(portfolio.getBaseAsset())
              + " Commissions And Fees:"
              + portfolioService.getBaseComissionAndFee(portfolio.getBaseAsset())
              + " Open Trade Equity:"
      ...
      for (Position position : portfolio.getNetPositions()) {
        ...
        log.debug(
            "Date: {} Portfolio: {} Instrument: {} Last trade time {} Last book time{}  Position: {} fills {}",
            (Timestamp != null ? (FORMAT.print(Timestamp)) : ""),
            portfolio,
            position.getAsset(),
            (quoteService.getLastTrade(position.getMarket()) == null
                ? ""
                : quoteService.getLastTrade(position.getMarket()).getTime()),
            (quoteService.getLastBook(position.getMarket()) == null
                ? ""
                : quoteService.getLastBook(position.getMarket()).getTime()),
            position,
            position.getFills());
      }
      ...
      //				logger.info("Date: " + (Timestamp != null ? (FORMAT.print(Timestamp)) : "") + " Asset: "
      // + postion.getAsset() + " Position: "
      //						+ postion.getVolume());
      //			}
    }
  }
  ...
  @Inject protected QuoteService quotes;
  @Inject protected PortfolioService portfolioService;
  @Inject protected BalanceFactory balanceFactory;
