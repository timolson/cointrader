...
@Cacheable
@NamedQuery(
    name = "Market.findByMarket",
    query = "select m from Market m where exchange=?1 and listing=?2",
    hints = {@QueryHint(name = "org.hibernate.cacheable", value = "false")})
@Table(
    indexes = {
      @Index(columnList = "exchange"),
...
public class Market extends Tradeable {
  ...
  @Inject protected static transient ExchangeFactory exchangeFactory;
  @Inject protected static transient MarketJpaDao marketDao;
  ...
  public static synchronized Market findOrCreate(Exchange exchange, Listing listing) {
    return findOrCreate(exchange, listing, listing.getPriceBasis(), listing.getVolumeBasis());
  }

  public static synchronized Market findOrCreate(
      Exchange exchange, Listing listing, double minOrderSize) {
    return findOrCreate(
        exchange, listing, listing.getPriceBasis(), listing.getVolumeBasis(), minOrderSize);
  }
  ...
  // @JoinTable(name = "market_synthetic_market", joinColumns = @JoinColumn(name = "market_id"),
  // inverseJoinColumns = @JoinColumn(name = "synthetic_id"))
  ...
  public synchronized void addSyntheticMarket(SyntheticMarket market) {
    synchronized (getSyntheticMarkets()) {
      if (!getSyntheticMarkets().contains(market)) getSyntheticMarkets().add(market);
    }
  }
  ...
  public static synchronized Market findOrCreate(
      Exchange exchange, Listing listing, double quoteBasis, double volumeBasis) {
    ...
    try {
      ...
      List<Market> results =
          EM.namedQueryList(Market.class, "Market.findByMarket", exchange, listing);
      if (results != null && !results.isEmpty() && results.get(0) != null) {
        for (Market result : results) {
          result.setPersisted(true);
          if (result != null) markets.add(result);
        }
  ...
  public static List<Market> find(Exchange exchange) {
    ...
    List<Market> markets =
        EM.queryList(
            Market.class, "select s from Market s where exchange=?1 and active=?2", exchange, true);

    for (Market market : markets) market.setPersisted(true);
    return markets;
  }
  ...
  public static List<Market> find(Listing listing) {
    List<Market> markets =
        EM.queryList(
            Market.class, "select s from Market s where listing=?1 and active=?2", listing, true);
    for (Market market : markets) market.setPersisted(true);
    return markets;
  }
  ...
  public double getVolumeBasis() {
    if (volumeBasis != 0) return volumeBasis;
    if (listing == null) return volumeBasis;
    return listing.getVolumeBasis() == 0 ? volumeBasis : listing.getVolumeBasis();
  }

  /**
   * @return true iff the Listing is currently traded at the Exchange. The Market could have been
   *     retired.
   */
  @Transient
  ...
  public Amount getBalance(Asset currency) {
    if (getExchange() != null
        && getExchange().getBalances() != null
        && getExchange().getBalances().get(currency) != null)
  ...
  public double getFeeRate(ExecutionInstruction executionInstruction) {
    return listing.getFeeRate(executionInstruction) == 0
        ? exchange.getFeeRate(executionInstruction)
        : listing.getFeeRate(executionInstruction);
  }

  @Transient
  public FeeMethod getMarginFeeMethod() {
    return listing.getMarginFeeMethod() == null
        ? exchange.getMarginFeeMethod()
        : listing.getMarginFeeMethod();
  }
  ...
  public double getMinimumOrderSize(Tradeable market) {
    if (minimumOrderSize == 0) return exchange.getMinimumOrderSize(market);
    else return minimumOrderSize;
  }
  ...
  public Market(
      ...
      @Assisted("marketPriceBasis") double priceBasis,
  ...
  public Market(
      ...
      @Assisted("marketPriceBasis") double priceBasis,
      @Assisted("marketVolumeBasis") double volumeBasis,
      @Assisted("minimumOrderSize") double minimumOrderSize) {
    this.exchange = exchange;
    this.listing = listing;
    this.priceBasis = priceBasis;
    this.volumeBasis = volumeBasis;
