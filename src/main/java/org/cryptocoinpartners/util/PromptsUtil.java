...
public class PromptsUtil {
  ...
  public static ZonedDateTime getExpiryDate(Market market, ZonedDateTime date) {
    return getExpiryDate(market.getListing(), date);
  }

  public static ZonedDateTime getExpiryDate(Listing listing, ZonedDateTime date) {
    ZonedDateTime expiryDate = date;
    if (listing.getPrompt() == null) return expiryDate;
    switch (listing.getPrompt().getSymbol()) {
      case "THISWEEK":
  ...
  private static ZonedDateTime getLastFridayOfQuarter(ZonedDateTime date) {
    ...
    ZonedDateTime lastFirdayOfQuarter =
        quater.with(TemporalAdjusters.lastInMonth(DayOfWeek.FRIDAY));
  ...
  private static ZonedDateTime getNextFriday(ZonedDateTime date) {
    ...
    if (dateHK.getDayOfWeek().equals(DayOfWeek.FRIDAY)
        && date.withZoneSameInstant(targetZone).getHour() < 16) nextFriday = dateHK;
    else nextFriday = dateHK.with(TemporalAdjusters.next(DayOfWeek.FRIDAY));
  ...
  private static ZonedDateTime getFridayAfterNext(ZonedDateTime date) {
    ...
    ZonedDateTime nextFriday;
    if (dateHK.getDayOfWeek().equals(DayOfWeek.FRIDAY)
        && date.withZoneSameInstant(targetZone).getHour() < 16)
