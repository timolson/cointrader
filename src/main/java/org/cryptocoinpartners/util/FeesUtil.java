...
public class FeesUtil {
  ...
  public static Amount getCommission(Fill fill) {
    ...
    switch (method) {
      ...
      case PercentagePerUnitOpening:
        commission = (fill.getOrder().getPositionEffect() == (PositionEffect.OPEN)) ? calculatePercentagePerUnit(price, ammount, rate,
  ...
  public static Amount getMargin(Position position) {
    if (position.isOpen()) {
      ...
      FeeMethod method = (position.getMarket().getMarginFeeMethod() == null) ? FeeMethod.PercentagePerUnit : position.getMarket()
          .getMarginFeeMethod();
