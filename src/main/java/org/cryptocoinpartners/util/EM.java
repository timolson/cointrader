 ...
 * </p>
 * <p>
 * Conventional wisdom is to inject the EntityManager into classes where you might need it. This breaks down when you need to perform database access
 * in places where injection is not available. For example, you may have a polymorphic hierarchy of entity objects that exhibit different data-access
 * behavior.
 * </p>
 * <p>
 * Aspects like persistence (aka transaction) and authorization really fit a thread-local model better. This static accessor for the EntityManager
 * gives you "always-available" access to the context.
 * </p>
 */
public class EM {
	@Inject
	...
	public static void queryEach(Visitor<Object[]> handler, int batchSize, String queryStr, Object... params) {
		...
		try {
			...
			log.error(EM.class.getClass().getSimpleName() + " - queryEach Unable to complete query " + queryStr + ", full stack trace follows:", ex);
	...
	public static <T> T namedQueryZeroOne(Class<T> resultType, String namedQuery, Object... params) {
		try {
			...
			log.error(EM.class.getClass().getSimpleName() + " - namedQueryZeroOne Unable to complete query " + namedQuery + " for  "
					+ resultType.getSimpleName() + ", full stack trace follows:", ex);
			return null;
		} finally {
			unitOfWork.end();
			log.trace("namedQueryZeroOne unit of work ended for thread: " + Thread.currentThread());
	...
	public static EntityBase find(Map<String, String> properties, EntityBase... entities) {
		...
		try {
			...
			if (foundEntity != null)
				...
				for (EntityBase entity : entities)
					log.info(entity.getClass().getSimpleName() + ": " + entity.getUuid().toString() + " not found in database");
			return foundEntity;
	...
	public static EntityBase find(EntityBase... entities) {
		...
		try {
			...
			if (foundEntity != null)
				for (EntityBase entity : entities)
					log.debug(entity.getClass().getSimpleName() + ": " + entity.getId().toString() + "/" + entity.getUuid().toString() + " found in database");

			else
				for (EntityBase entity : entities)
					log.info(entity.getClass().getSimpleName() + ": " + entity.getId().toString() + "/" + entity.getUuid().toString()
	...
	public static <T> T queryZeroOne(Class<T> resultType, String queryStr, Object... params) {
		try {
			...
			log.error(EM.class.getClass().getSimpleName() + " - queryZeroOne Unable to complete query " + queryStr + " for  " + resultType.getSimpleName()
	...
	public static <T> T sqlQueryOne(Class<T> resultType, String queryStr, Object... params) throws NoResultException {
		...
		try {
			...
			log.error(EM.class.getClass().getSimpleName() + " - sqlQueryOne Unable to complete query " + queryStr + " for  " + resultType.getSimpleName()
					+ ", full stack trace follows:", ex);
			return null;
	...
	public static <T> T sqlQueryLimitOne(Class<T> resultType, String queryStr, Object... params) throws NoResultException {
		//   EntityManager em = em();
		T result = null;
		try {
			beginUnitOfWork();
			...
			log.error(EM.class.getClass().getSimpleName() + " - sqlQueryLimitOne Unable to complete query " + queryStr + " for  " + resultType.getSimpleName()
					+ ", full stack trace follows:", ex);
			return null;
	...
	public static <T> List<T> namedQueryList(Class<T> resultType, String namedQuery, Object... params) {
		try {
			...
			log.error(EM.class.getClass().getSimpleName() + " - namedQueryList Unable to complete query " + namedQuery + " for  " + resultType.getSimpleName()
	...
	public static <T> List<T> queryList(Class<T> resultType, String queryStr, Object... params) {
		...
		try {
			...
			log.error(EM.class.getClass().getSimpleName() + " - queryList Unable to complete query " + queryStr + " for  " + resultType.getSimpleName()
					+ ", full stack trace follows:", e);
	...
	public static <T> void queryEach(Class<T> resultType, Visitor<T> handler, int batchSize, String queryStr, Object... params) {
		try {
			...
			log.error(EM.class.getClass().getSimpleName() + " - queryEach Unable to complete query " + queryStr + " for  " + resultType.getSimpleName()
					+ ", full stack trace follows:", ex);
