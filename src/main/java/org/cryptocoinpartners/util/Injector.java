...
/**
 * Guice doesn't allow binding overrides so we have to store the Configuration separately and add it
 * in just before instance creation My first time using Guice... not a fan. Should have tried Pico.
 *
 * @author Tim Olson
 */
public class Injector {
  ...
  public Injector createChildInjector(
      java.lang.Iterable<? extends com.google.inject.Module> iterable) {
  ...
  static {
    ...
    properties.put(
        "hibernate.jdbc.fetch_size", ConfigUtil.combined().getString("db.fetch_size", "10000"));
    properties.put("hibernate.connection.release_mode", "auto");
    // properties.put("hibernate.jdbc.batch_size", ConfigUtil.combined().getString("db.batch_size",
    // "1000000000"));
    ...
    properties.put(
        "hibernate.c3p0.maxIdleTime", ConfigUtil.combined().getString("db.max_idle_time", "0"));
    properties.put(
        "hibernate.c3p0.idle_test_period",
        ConfigUtil.combined().getString("db.idle.test.period", "0"));
    ...
    properties.put(
        ...
        ConfigUtil.combined().getString("db.acquire_retry_delay", "5000"));
    properties.put(
        "hibernate.c3p0.acquireRetryAttempts",
        ConfigUtil.combined().getString("db.acquire_retry_attempts", "30"));
    properties.put(
        "hibernate.c3p0.breakAfterAcquireFailure",
        ConfigUtil.combined().getString("db.break_after_acquire_failure", "false"));
    properties.put(
        "hibernate.c3p0.checkoutTimeout",
        ConfigUtil.combined().getString("db.checkout_timeout", "0"));
    properties.put(
        "hibernate.c3p0.idleConnectionTestPeriod",
        ConfigUtil.combined().getString("db.idle_connection_test_period", "0"));
    properties.put(
        "hibernate.c3p0.numHelperThreads",
        ConfigUtil.combined().getString("db.num_helper_threads", "10"));
    properties.put(
        "hibernate.c3p0.unreturnedConnectionTimeout",
        ConfigUtil.combined().getString("db.unreturned_connection_timeout", "0"));
    properties.put(
        "hibernate.c3p0.statementCacheNumDeferredCloseThreads",
        ConfigUtil.combined().getString("db.statement_cache_num_deferred_close_threads", "1"));
    properties.put(
        "hibernate.c3p0.maxAdministrativeTaskTime",
        ConfigUtil.combined().getString("db.max_administrative_task_time", "0"));
    ...
    //  properties.put("hibernate.c3p0.max_statements", "0");", "false"));.
    // debug_unreturned_connection_stack_traces
    //  properties.put("hibernate.c3p0.max_statements", "0");
    properties.put(
        "hibernate.c3p0.maxStatementsPerConnection",
        ConfigUtil.combined().getString("db.max_statements_per_connection", "0"));
    properties.put(
        "hibernate.c3p0.statementCacheNumDeferredCloseThreads",
        ConfigUtil.combined().getString("db.statement_cache_num_deferred_close_threads", "1"));
    properties.put(
        "hibernate.c3p0.maxStatements", ConfigUtil.combined().getString("db.max_statements", "0"));
    // properties.put("hibernate.c3p0.validate", "true");

    properties.put("javax.persistence.sharedCache.mode", "NONE");
    properties.put(
        "javax.persistence.query.timeout",
        ConfigUtil.combined().getString("db.query_timeout", "9999999"));

    properties.put("javax.persistence.LockModeType", "OPTIMISTIC");

    // root = new Injector(Guice.createInjector(new LogInjector(), new PersistanceModule()),
    // ConfigUtil.combined());
    ...
    root =
        new Injector(
            Guice.createInjector(
                new LogInjector(),
                new JpaPersistModule("org.cryptocoinpartners.schema").properties(properties),
