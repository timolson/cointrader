...
public class OHLCBarPlugInViewFactory extends ViewFactorySupport {
  protected ViewFactoryContext viewFactoryContext;
  ...
  public void setViewParameters(
      ViewFactoryContext viewFactoryContext, List<ExprNode> viewParameters)
  ...
  public void attach(
    ...
    ExprNode[] validatedNodes =
        ViewFactorySupport.validate(
            "OHLC view", parentEventType, statementContext, viewParameters, false);
    ...
    if ((volumeExpression.getExprEvaluator().getType() != double.class)
        && (volumeExpression.getExprEvaluator().getType() != Double.class)) {
      throw new ViewParameterException("View requires double-typed values for in parameter 3");
    }
    if ((marketExpression.getExprEvaluator().getType() != Tradeable.class)
        && (marketExpression.getExprEvaluator().getType() != Tradeable.class)) {
      throw new ViewParameterException("View requires tradeable-typed values for in parameter 4");
    }
    if ((intervalExpression.getExprEvaluator().getType() != double.class)
        && (intervalExpression.getExprEvaluator().getType() != Double.class)) {
