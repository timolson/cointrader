...
public class BasicQuoteService implements QuoteService {
	...
	public Bar getLastBar(Tradeable market, double interval) {
		if (market == null || lastBarByMarket == null || lastBarByMarket.get(market.getSymbol()) == null
				|| lastBarByMarket.get(market.getSymbol()).get(interval) == null)
			return null;
		/*
		 * if (XchangeData.exists()) { XchangeData xchangeData = context.getInjector().getInstance(XchangeData.class); try { for (Trade trade :
		 * xchangeData.getTrades(market, market.getExchange())) recordTrade(trade); } catch (Throwable e) { // TODO Auto-generated catch block
	...
	private final Map<String, Map<Double, Bar>> lastBarByMarket = new ConcurrentHashMap<String, Map<Double, Bar>>();
	private final Map<String, Map<Double, Bar>> lastBarByListing = new ConcurrentHashMap<String, Map<Double, Bar>>();
