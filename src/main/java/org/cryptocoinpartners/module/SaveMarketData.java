...
public class SaveMarketData {
  ...
  @Inject protected BarFactory barFactory;

  @Inject protected HeikinAshiBarFactory heikinAshiBarFactory;

  @Inject protected BookJpaDao bookDao;

  @Inject protected TradeJpaDao tradeDao;

  @Inject protected MarketDataJpaDao marketDataDao;

  @Inject protected BarJpaDao barDao;
  ...
  */
  ...
  public void handleBook(Book m) {
    ...
    try {
      if (m.getDao() == null) {
        log.debug("SaveTradeRunnable:saveData - Injecting dao");
        context.getInjector().injectMembers(m);
      }
      if (m.getParent() != null && m.getParent().getDao() == null) {
        log.debug("SaveTradeRunnable:saveData - Injecting dao");
        context.getInjector().injectMembers(m.getParent());
      }
      m.persit();
  ...
  public void handleBar(Bar m) {
    ...
    try {
      if (m.getDao() == null) context.getInjector().injectMembers(m);
      m.persit();
    } catch (Error | Exception ex) {
