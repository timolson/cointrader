...
public class LiveTradeRunMode extends RunMode {
  ...
  List<String> barIntervals =
      ConfigUtil.combined().getList("strategy.prefeed.bar.intervals", new ArrayList<String>());
  ...
  public void run(Semaphore semaphore) {
    ...
    Replay replay =
        replayFactory.between(
            start, end, false, paperSemaphore, false, replayBooks, replayBars, barIntervals);
    ...
    orderService.setTradingEnabled(
        true); //  context.publish(new
               // TimerControlEvent(TimerControlEvent.ClockType.CLOCK_INTERNAL));
